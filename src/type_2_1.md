...
10
```
このように、文字列や配列もポインタ型として扱います。文字列リテラルはダブルクォーテーションで囲みます。また、配列は先に型と要素数を指定して`alloc`でスタック上にメモリを確保して`Array`で値を列挙して配列に格納していき、`[](ポインタ変数, インデックス)`で値を取り出せます。`[]`では多次元配列でも`[](ポインタ変数, インデックス1, インデックス2, ...)`のようにして簡単に値を取り出すことができます。